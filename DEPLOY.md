...
```
GitHub (main branch)
  ├── push em backend/  → GitHub Actions → Fly.io (aps-edu)
  └── push em web-admin/ → GitHub Actions → Fly.io (aps-edu-web)
```

URLs finais:
- **API**: https://aps-edu.fly.dev
- **Painel Web**: https://aps-edu.vercel.app
- **Docs API**: https://aps-edu.fly.dev/docs
...
```bash
cd backend
fly apps create aps-edu
```

Conectar ao banco:
```bash
fly postgres attach aps-edu-db --app aps-edu
```

Configurar as variáveis de ambiente (secrets):
```bash
fly secrets set --app aps-edu \
...
```bash
cd backend
fly ssh console --app aps-edu -C "node src/prisma/seed.js"
```
...
fly tokens create deploy --app aps-edu --name "github-actions-backend"
...
fly status --app aps-edu

# Ver logs em tempo real
fly logs --app aps-edu
...
Acesse:
- API Health: https://aps-edu.fly.dev/health
- Swagger Docs: https://aps-edu.fly.dev/docs
- Painel Web: https://aps-edu.vercel.app
...
// Por:
const String kBaseUrl = 'https://aps-edu.fly.dev/api';
```
...
fly ssh console --app aps-edu

# Ver variáveis de ambiente configuradas
fly secrets list --app aps-edu

# Escalar máquina (se precisar de mais recursos)
fly scale vm shared-cpu-1x --memory 1024 --app aps-edu

# Reiniciar o app
fly apps restart aps-edu
```
