...
const ACTIONS = [
  ...
  { id: 'sofi_summary',       label: 'IA da Educação gera resumo semanal',         icon: '◆' },
  { id: 'sofi_risk_radar',    label: 'IA da Educação atualiza Radar de Unidade',   icon: '◆' },
...
const PRESET_RULES: Omit<AutomationRule, 'id' | 'runs' | 'lastRun' | 'createdAt'>[] = [
  ...
  {
    name: 'x& Lembrete de evento próximo',
    trigger: 'event_1h',
    condition: 'any',
    action: 'notify_assignee',
    active: true,
  ...
  {
    name: 'x Resumo semanal da IA da Educação',
  ...
  {
    name: 'x E-mail para baixo engajamento',
  ...
  {
    name: 'x Radar APS de unidade em risco',
    trigger: 'daily_9am',
  ...
  {
    name: 'x} Follow-up automático de atraso',
    trigger: 'task_overdue',
...
export default function AutomaçõesPage() {
  ...
  const runNow = async (rule: AutomationRule) => {
    ...
    saveRules(updated)
    setRunning(null)
    success('S& Automação executada!', `"${rule.name}" rodou com sucesso.`)
  }

  const getTriggerLabel = (id: string) => TRIGGERS.find(t => t.id === id)?.label || id
  const getTriggerIcon  = (id: string) => TRIGGERS.find(t => t.id === id)?.icon || 'a'
  const getActionLabel  = (id: string) => ACTIONS.find(a => a.id === id)?.label || id
  const getActionIcon   = (id: string) => ACTIONS.find(a => a.id === id)?.icon || 'x'
          ...
          >
            <div className="p-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
              <h2 className="text-base font-bold text-white">
                {editing ? 'S️ Editar Automação' : 'a Nova Automação'}
              </h2>
            ...
            <div className="p-5 space-y-4">
              ...
              {form.trigger && form.action && (
                ...
                >
                  <p className="text-xs font-semibold mb-2" style={{ color: '#F8A303' }}>S Pré-visualização</p>
            ...
            <div className="flex gap-3 p-5" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              >
                {editing ? 'x Salvar Alterações' : 'a Criar Automação'}
              </button>
            </div>
          </div>
        </div>
