...
function parsePersonalTasksFromText(text: string): Partial<PersonalTask>[] {
  ...
  return pieces.slice(0, 12).map(raw => {
    ...
    const category: PersonalTask['category'] = /campanha|tiktok|marketing|drone|conte[úu]do|postar/.test(lower)
      ? 'campanha'
...
function WorkDayTimer({ tasks, workDay: extWd, onWorkDayUpdated }: {
  ...
  // Sync com IA da Educação em tempo real
  useEffect(() => {
    if (extWd) {
        ...
        <div className="flex items-center gap-2">
          ...
          {flash && (
            <span className="text-[10px] px-2 py-0.5 rounded-full animate-pulse"
              ...
              S Atualizado pela IA da Educação
            </span>
          )}
        </div>
...
function normalizeNotebookEntry(entry: NotebookEntry): NotebookEntry {
  ...
  const oldEmptyTitles = ['Nova anotação', 'Nova anotação']
  return {
    ...entry,
    type,
    title: oldEmptyTitles.includes(entry.title) ? notebookTitle(type) : entry.title.replace('anotação', 'anotação'),
...
function IntelligentNotebook() {
  ...
  const [sofiPanel, setSofiPanel] = useState<{ title: string; content: string; status: 'idle' | 'loading' | 'done' | 'error' }>({
    title: 'Resposta da IA da Educação',
    content: '',
    status: 'idle',
  })
  ...
  const organizeWithSofi = async () => {
    ...
    setSofiPanel({
      ...
      content: 'A IA da Educação está lendo sua anotação e organizando a resposta...',
      status: 'loading',
    })

    const prompt = `Você é a IA da Educação, assistente executiva da Associação Paulista Sul. Organize esta anotação como um caderno digital de alto nível, com clareza executiva, decisões rastreáveis e próximos passos acionáveis.
...
Versão refinada da anotação`
    ...
    try {
      ...
      const output = data.content || data.error || 'A IA da Educação não conseguiu organizar esta anotação agora.'
      ...
      setSaveHint('Enviado para IA da Educação')
      window.setTimeout(() => setSaveHint(''), 1800)
    } catch (err: any) {
      const output = `Erro ao chamar a IA da Educação: ${err.message}`
      const completed: NotebookEntry = { ...currentDraft, aiOutput: output, updatedAt: new Date().toISOString() }
      const withoutCompleted = entries.filter(item => item.id !== completed.id)
  ...
  const askSofiToAdjust = async () => {
    ...
    setSofiPanel(prev => ({
      ...prev,
      content: `${prev.content}\n\n---\nAjustando com a IA da Educação: ${instruction}`,
      status: 'loading',
    }))

    const prompt = `Você é a IA da Educação, assistente executiva da Associação Paulista Sul.
...
Reescreva a resposta de forma melhor, mais organizada e pronta para uso. Use português do Brasil, títulos curtos, bullets claros, decisões, próximos passos e tarefas quando fizer sentido.`
    ...
    try {
      ...
      const data = await res.json()
      const output = data.content || data.error || 'A IA da Educação não conseguiu ajustar a resposta agora.'
      setSofiPanel(prev => ({ ...prev, content: output, status: data.error ? 'error' : 'done' }))
      setSofiInstruction('')
    } catch (err: any) {
      setSofiPanel(prev => ({ ...prev, content: `Erro ao ajustar com a IA da Educação: ${err.message}`, status: 'error' }))
    } finally {
      setAiBusy(false)
    }
  }
        ...
        <aside className="p-4 border-b lg:border-b-0 lg:border-l lg:order-2"
          ...
          <div className="flex items-center justify-between gap-2 mb-4">
            <div>
              ...
              <p className="text-[10px] text-white/35">Notas, ideias, frases e planos com IA da Educação</p>
            </div>
          ...
          <div className="rounded-2xl p-3 mb-4"
            ...
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className="min-w-0">
                <p className="text-sm font-extrabold text-white truncate">IA da Educação ao lado</p>
              ...
              <>
                ...
                <div className="mt-3 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                  <textarea
                    ...
                    placeholder="Peça para a IA da Educação corrigir, resumir, deixar mais executivo, transformar em tarefas..."
                  ...
                  <button onClick={askSofiToAdjust} disabled={aiBusy || !sofiInstruction.trim()}
                    ...
                    style={{ background: 'linear-gradient(135deg,#F8A303,#FDC347)' }}>
                    {aiBusy ? 'IA da Educação ajustando...' : 'Pedir ajuste à IA da Educação'}
                  </button>
                </div>
              </>
            ) : (
              <div className="h-[120px] flex items-center justify-center text-center text-xs text-white/28 px-3">
                Envie a anotação para a IA da Educação e a resposta aparecerá aqui. O editor será limpo para o próximo preenchimento.
              </div>
            )}
          </div>
            ...
            <div>
              ...
              <div className="flex justify-end mt-3">
                <button onClick={organizeWithSofi} disabled={aiBusy}
                  ...
                  <BoltIcon className="w-4 h-4" /> {aiBusy ? 'Enviando para IA da Educação...' : 'Enviar para IA da Educação'}
                </button>
              </div>
              <div className="flex items-center gap-2 flex-wrap mt-3 rounded-2xl p-3"
                ...
                {[
                  ...
                  { label: 'IA da Educação', value: draft.aiOutput ? 'analisado' : 'pendente' },
                ].map(item => (
    ...
    return (
      <div className="rounded-2xl p-8 flex flex-col items-center gap-4"
        ...
        <p className="text-[10px] text-center max-w-xs" style={{ color: 'rgba(255,255,255,0.2)' }}>
          x Dados criptografados neste dispositivo com chave derivada do PIN. Nunca enviados ao servidor.
        </p>
      </div>
    )
  }
...
function AiAssistantPanel({ tasks, workDay, userName, onTaskCreated, onEventCreated, onWorkDayUpdated, onWorkspaceRefresh }: {
  ...
  function getFileEmoji(file: File) {
    if (file.type.startsWith('image/')) return 'x'
    if (file.type.startsWith('audio/')) return 'x}'
    if (file.type.startsWith('video/')) return 'x}'
    if (file.type.includes('pdf')) return 'x"'
    return 'x'
  }
  ...
  useEffect(() => {
    ...
    api.post('/ai/chat', {
      messages: [{ role: 'user', content: `${period}! ${promptCtx} Me cumprimente pelo nome (${userName}), se apresente como IA da Educação e pergunte ate que horas vou trabalhar hoje. Max 2 linhas.` }],
  ...
  const executeAction = async (action: { type: string; data: any }) => {
    try {
      if (action.type === 'create_task') {
        ...
        setMessages(p => [...p, { role: 'assistant', content: `S& Tarefa criada e destacada: **${res.data?.title || action.data?.title || 'Nova tarefa'}**` }])
        onTaskCreated?.()
        ...
        if (created.length) {
          setMessages(p => [...p, { role: 'assistant', content: `S& ${created.length} tarefas criadas e destacadas:\n${created.map((t, i) => `${i + 1}. ${t.title}`).join('\n')}` }])
          onTaskCreated?.()
        }
      ...
      //  BUSCA NA INTERNET 
        ...
        try {
          ...
          setMessages(p => [...p, { role: 'assistant', content: `x **Pesquisa: "${action.data.query}"**\n\n${summary}` }])
        } catch {
          setMessages(p => [...p, { role: 'assistant', content: 'R Não consegui pesquisar na internet agora.' }])
        }
        setLoading(false)
        ...
        try {
          ...
          if (imgData.imageBase64) {
            ...
            setMessages(p => [...p, {
              ...
              display: `x} Imagem gerada!`,
            }])
          } else {
            setMessages(p => [...p, { role: 'assistant', content: `R Não consegui gerar a imagem: ${imgData.error}` }])
          }
        } catch {
          setMessages(p => [...p, { role: 'assistant', content: 'R Erro ao gerar imagem.' }])
        }
        setLoading(false)
      }
    } catch (e) {
  ...
  const send = async (override?: string) => {
    ...
    if (file) {
      ...
      if (processed.imageBase64) {
        ...
        finalText = baseText || 'Análise esta imagem detalhadamente e descreva o que vê. Se houver texto, leia-o. Se for um documento, extraia as informações principais.'
        ...
        let instruction = ''
        if (isAudio) {
          instruction = `\n\n---\nIA DA EDUCAO, voc recebeu a transcrição de um áudio do usuário. Análise o conteúdo transcrito: identifique o tema principal, pontos-chave e contexto. ${baseText ? 'O usuário também disse: ' + baseText : 'Responda de forma natural como se tivesse ouvido o áudio, resuma o conteúdo e pergunte se precisa de algo.'}`
        } else if (isDoc) {
          instruction = `\n\n---\nIA DA EDUCAO, voc acabou de receber e ler o conteúdo completo do documento acima. Faça uma análise profissional e inteligente: (1) Resumo executivo em 3-5 pontos principais, (2) Informações mais relevantes e dados importantes encontrados, (3) Pergunte se o usuário quer algo específico sobre o conteúdo. ${baseText ? 'O usuário também pediu: ' + baseText : ''} Responda como uma secretária profissional e eficiente.`
        }
    ...
    if (file) {
      const icon = file.type.startsWith('image/') ? 'x️' : file.type.startsWith('audio/') ? 'x}' : 'x}'
    ...
    if (createsTasks && !file) {
      ...
      if (parsedTasks.length > 1) {
        try {
          ...
          const answer = `S& ${created.length} tarefas criadas, destacadas e colocadas no calendário da sua área quando têm prazo:\n${created.map((t, i) => `${i + 1}. ${t.title}`).join('\n')}`
          setMessages(p => [...p, { role: 'assistant', content: answer }])
          speakText(answer)
        } catch (err: any) {
          const msg = err?.response?.data?.error || err?.message || 'Erro ao criar tarefas'
          setMessages(p => [...p, { role: 'assistant', content: `R ${msg}` }])
        }
        setLoading(false)
        return
      }
      ...
      <div ref={chatContainerRef} className="flex-1 overflow-y-auto space-y-3 p-4 rounded-2xl mb-3"
        ...
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full gap-4">
            ...
            <p className="text-sm" style={{ color: 'rgba(255,255,255,0.3)' }}>Iniciando IA da Educação...</p>
          </div>
        )}
        {messages.map((m, i) => (
            ...
            <div className="max-w-[80%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap"
              ...
              {(() => {
                ...
                if (txt.startsWith('[IMAGE:')) {
                  ...
                  return (
                    <div>
                      <img src={src} alt="Imagem gerada pela IA da Educação" className="rounded-xl max-w-full" style={{ maxHeight: 320 }} />
                      <p className="text-[10px] mt-1.5" style={{ color: 'rgba(255,255,255,0.4)' }}>✨ Gerado pela IA da Educação</p>
                    </div>
                  )
                }
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title={recognitionRef.current ? (isListening ? 'Parar gravação' : 'Falar para IA da Educação') : 'Voz não suportada neste navegador'}
          ...
          <button
            ...
            title={voiceEnabled ? 'Desativar voz da IA da Educação' : 'Ativar voz da IA da Educação'}
            ...
            {voiceEnabled ? 'x` Voz ON' : 'x! Voz OFF'}
          </button>
          {isSpeaking && (
          ...
          <span className="ml-auto text-[10px]" style={{ color: 'rgba(255,255,255,0.15)' }}>
            {voiceEnabled ? 'x}"️ IA da Educação falar as respostas' : 'Clique x! para ativar voz'}
          </span>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <input type="text" value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder={isListening ? 'x} Ouvindo...' : 'Pergunte sobre tarefas, agenda, campanha, promotores...'}
              ...
              {editMode ? (
                <>
                  <button onClick={saveEdit} disabled={saving}
                    ...
                    style={{ background: 'linear-gradient(135deg,#F8A303,#FDC347)' }}>
                    {saving ? 'Salvando...' : 'S& Salvar alterações'}
                  </button>
        ...
        <div className="p-3 space-y-1.5">
          ...
          : emails.length === 0 ? (
            <div className="text-center py-4">
              <p className="text-xs" style={{ color: 'rgba(255,255,255,0.2)' }}>S& Nenhum email não lido</p>
            </div>
          ) : emails.map(e => (
            <a key={e.id}
...
export default function MinhaAreaPage() {
  ...
  const updateTask = async (id: string, upd: Partial<PersonalTask>) => {
    ...
    try {
      ...
      if (upd.status === 'done') {
        ...
        sendNotif('S& Tarefa concluída!', `"${t?.title}"  +${t?.xp} XP`)
      }
    } catch { loadTasks() }
  }
  ...
  return (
    <AdminLayout>
      ...
      {/* """"""""""""""""""""""""""""""""""""""""""""""""""""""
           SE!ÒO 1  CALENDÁRIO GOOGLE (destaque principal)
      """""""""""""""""""""""""""""""""""""""""""""""""""""" */}
      ...
      {/* """"""""""""""""""""""""""""""""""""""""""""""""""""""
           SE!ÒO 2  IA DA EDUCAO + TAREFAS (lado a lado)
      """""""""""""""""""""""""""""""""""""""""""""""""""""" */}
      ...
      <section className="mb-6 animate-fade-in-up">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            ...
            <p className="text-[10px] mt-0.5" style={{ color: 'rgba(255,255,255,0.3)' }}>Notas estilo Notion com IA IA da Educação</p>
          </div>
        </div>
        <IntelligentNotebook />
      </section>
      ...
      {/* """"""""""""""""""""""""""""""""""""""""""""""""""""""
           SE!ÒO 4  GOOGLE WORKSPACE (Gmail + Drive)
      """""""""""""""""""""""""""""""""""""""""""""""""""""" */}
      <section className="mb-6 animate-fade-in-up">
        <div className="flex items-center gap-2 mb-3">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center text-base"
      ...
      {/* """"""""""""""""""""""""""""""""""""""""""""""""""""""
           SE!ÒO 5  CONQUISTAS + CREDENCIAIS (lado a lado)
      """""""""""""""""""""""""""""""""""""""""""""""""""""" */}
