...
function Surface({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <section className={`sofi-ops-surface rounded-lg border border-[#E0E7EF] bg-white shadow-[0_10px_30px_rgba(15,42,74,0.07)] ${className}`}>{children}</section>
}

function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={`h-11 w-full rounded-lg border border-[#DCE5EF] bg-white px-3 text-sm font-semibold text-[#17233B] outline-none transition placeholder:text-[#8A98AA] focus:border-[#0877C9] focus:ring-4 focus:ring-[#0877C9]/10 ${props.className || ''}`} />
}
...
export default function WorldClassOperations({
  ...
  return (
    <div className="space-y-5">
      ...
      {activeView !== 'pessoas' && (
        <form onSubmit={addQuickWork} className="sofi-quick-add grid gap-2 rounded-lg border border-[#DCE5EF] bg-white p-3 shadow-[0_10px_28px_rgba(15,42,74,0.06)] xl:grid-cols-[minmax(260px,1.2fr)_180px_180px_180px_48px]">
...
function CentralOperationalWorkspace({
  ...
  return (
    <section className="space-y-5">
      ...
      {activeView === 'kanban' && (
        <Surface className="sofi-kanban-shell overflow-hidden">
          <SectionHeader
            eyebrow="Execucao"
            title="Kanban profissional de tarefas e projetos"
            action={
              <div className="flex flex-wrap items-center gap-2">
                <div className="inline-flex rounded-lg border border-[#DCE5EF] bg-[#F4F6FA] p-1">
                  {([
      ...
      {boardView === 'kanban' ? (
            <div className="sofi-kanban-board overflow-x-auto p-4 sm:p-5">
...
function KanbanColumn({
  ...
  return (
    <div
      className="sofi-kanban-column w-[308px] shrink-0 rounded-lg border border-[#E0E7EF] bg-[#F2F5F8] p-2 shadow-[0_8px_24px_rgba(15,42,74,0.05)]"
    ...
    >
      <div
        className="rounded-md border border-[#E0E7EF] bg-white p-2.5"
        style={{ boxShadow: `inset 3px 0 0 ${column.color}` }}
      >
        <div className="flex items-start gap-2.5">
          <button
            ...
            className="mt-0.5 flex h-8 w-8 cursor-grab items-center justify-center rounded-md border border-transparent bg-transparent text-[#90A0B3] transition hover:border-[#DCE5EF] hover:bg-[#F5F7FA] hover:text-[#334155] active:cursor-grabbing"
          ...
          <div className="min-w-0 flex-1">
            <Input
              ...
              className="h-9 rounded-md border-transparent bg-transparent px-2 text-sm font-black tracking-wide text-[#17233B] shadow-none hover:border-[#DCE5EF] focus:bg-white"
            />
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <span className="h-2 w-2 rounded-full" style={{ background: column.color }} />
              <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#8492A5]">Arraste para reorganizar</p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-2">
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-md border border-[#DCE5EF] bg-white transition hover:bg-[#F5F7FA]"
              aria-label={`Alterar cor de ${column.title}`}
            >
              <span className="h-3.5 w-3.5 rounded-full border border-white" style={{ background: column.color, boxShadow: '0 0 0 1px #DCE5EF' }} />
            </button>
            <span className="min-w-7 rounded-full bg-[#EEF2F6] px-2 py-1 text-center text-[11px] font-black text-[#53647A]">{items.length}</span>
          </div>
        </div>
      </div>

      <div className="mt-2 space-y-2">
        {items.length === 0 && <p className="rounded-md border border-dashed border-[#CFD9E4] bg-white/60 p-5 text-center text-xs font-bold text-[#8A98AA]">Solte um card aqui</p>}
        {items.map(item => {
          ...
          return (
            <button
              ...
              className="sofi-kanban-card group w-full rounded-lg border border-[#E0E7EF] bg-white p-3 text-left shadow-[0_2px_7px_rgba(15,42,74,0.06)] transition hover:-translate-y-0.5 hover:border-[#C7D5E3] hover:shadow-[0_8px_18px_rgba(15,42,74,0.10)]"
            >
              <div className="mb-3 h-1 rounded-full" style={{ background: accent }} />
              <div className="flex items-start justify-between gap-2">
                <p className="text-[14px] font-black leading-snug text-[#17233B]">{item.title}</p>
                <span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: accent, background: `${accent}18` }}>
                  {item.priority}
                </span>
              </div>

              <div className="mt-2 flex items-center gap-2 text-xs font-semibold text-[#64748B]">
                <span>{item.owner}</span>
                <span className="text-[#B1BCC9]">·</span>
                <span>{item.project || item.area}</span>
              </div>

              {item.description && <p className="mt-3 line-clamp-2 text-sm leading-5 text-[#5F7085]">{item.description}</p>}

              <div className="mt-3 flex flex-wrap items-center gap-1.5 text-xs font-bold text-[#64748B]">
                <span className="rounded-md bg-[#F2F5F8] px-2 py-1 text-[11px] font-black text-[#45566C]">{item.due}</span>
                {checklistTotal ? (
                  <span className="inline-flex items-center gap-1 rounded-md bg-[#F2F5F8] px-2 py-1 text-[11px] font-black text-[#53647A]">
                ...
                {item.tags?.slice(0, 3).map(tag => (
                  <span key={tag} className="rounded-md border border-[#E1E7EE] bg-[#FAFBFC] px-2 py-1 text-[10px] font-black text-[#64748B]">{tag}</span>
                ))}
              </div>
              ...
              {item.participants?.length ? (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {item.participants.slice(0, 3).map(name => (
                    <span key={name} className="inline-flex h-7 items-center justify-center rounded-full border border-[#DCE5EF] bg-[#F5F8FB] px-2.5 text-[10px] font-black text-[#45566C]">
                      {name}
                    </span>
                  ))}
                </div>
