...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className={styles.shell}>
    ...
    <aside className={`${styles.sidebar} ${mobile ? styles.open : ''}`}>
      <Link href="/dashboard" className={styles.brand} aria-label="SOFI Educação Adventista"><span className={styles.brandIcon}><img src="/icon-ea.svg" alt="" /></span><div><strong>SOFI<span>APS EDU</span></strong><small>Sistema de gestão educacional</small></div></Link>
      <button className={styles.workspaceSwitch} onClick={() => setPalette(true)}><span className={styles.workspaceMark}>A</span><span>Ambiente APS<small>Operação, pessoas e estudos</small></span><ChevronDownIcon /></button>
    ...
    <div className={styles.body}>
      <header className={styles.topbar}><div className={styles.breadcrumb}><button className={styles.menuButton} aria-label="Abrir navegação" onClick={() => setMobile(true)}><Bars3Icon /></button><Squares2X2Icon /><span>Espaço de trabalho</span><ChevronRightIcon /><b>{current?.name || 'APS EDU'}</b></div><div className={styles.topActions}><span className={styles.systemStatus}><i />Conectado</span><button className={styles.search} onClick={() => setPalette(true)} aria-label="Buscar na plataforma"><MagnifyingGlassIcon /><span>Buscar ou navegar</span><kbd>Ctrl K</kbd></button><Link href="/notificacoes" title="Notificações" aria-label="Notificações"><BellIcon /></Link><Link href="/configuracoes" className={styles.avatar} title="Minha conta">{user?.name?.[0] || 'A'}</Link></div></header>
