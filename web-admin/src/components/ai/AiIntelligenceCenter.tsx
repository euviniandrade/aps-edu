...
function actionLabel(type?: string) {
  const map: Record<string, string> = {
    ...
    create_event: 'Evento de calendário',
    create_ai_artifact: 'Documento / artefato',
    create_management_work: 'Item de gestão',
    create_note: 'Nota executiva',
  }
  return map[type || ''] || 'Ação operacional'
}
...
export default function AiIntelligenceCenter() {
  ...
  async function executeAction(action: SofiAction): Promise<ActionExecutionResult> {
    ...
    if (action.type === 'create_tasks') {
      ...
      return { message: `${created.length} tarefa(s) criadas na sua área.`, route: '/minha-area' }
    }
    ...
    if (action.type === 'create_management_work') {
      ...
      return { message: `Item criado na gestão: ${item.title}`, route: '/gestao?view=kanban' }
    }
    ...
    if (action.type === 'create_note') {
      ...
      const note = {
        id: `note-${Date.now()}`,
        title: action.data?.title || 'Nota da IA da Educação',
  ...
  async function sendMessage(prefilled?: string) {
    ...
    try {
      ...
      const contextParts = [
        ...
        integrations.length
          ? `Integrações externas: ${integrations.map(item => `${item.name}=${item.connected ? 'conectado' : item.envReady ? 'pronto para conectar' : 'nao configurado'}`).join('; ')}`
          : '',
      ].filter(Boolean)
  ...
  function renderActionDetails(action?: SofiAction | null) {
    ...
    const detailsByType: Record<string, Array<{ label: string; value?: string }>> = {
      ...
      create_tasks: [
        ...
        { label: 'Destino', value: 'Minha área' },
      ],
      create_event: [
                              ...
                              <div className="flex items-center justify-between gap-3">
                                <div>
                                  ...
                                  <p className="mt-1 text-sm font-medium text-white">
                                    {message.actionSummary || 'Ação processada pela IA da Educação.'}
                                  </p>
                                </div>
